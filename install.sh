...
# Copies this directory to /mnt/etc/nixos, adds sibling st/ blobs into the
# target store (requireFile), nixos-installs #blackwell.
# After reboot: Sway, then bash /etc/nixos/bootstrap.sh
set -euo pipefail
...
# nixos-install builds with --store /mnt. requireFile is flat sha256 + basename.
